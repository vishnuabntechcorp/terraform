...
variable "instance_type" {
  ...
  validation {
    condition     = can(regex("^t2\\.[a-z]+$", var.instance_type))
    error_message = "Instance type must follow the pattern 't2.<type>', e.g., 't2.micro'."
  }
}
...
variable "sandboxes" {
  ...
  validation {
    condition     = alltrue([for sandbox in var.sandboxes : length(sandbox) > 0])
    error_message = "All sandbox names must be non-empty strings."
  }
}
